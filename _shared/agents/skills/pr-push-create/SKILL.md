---
name: pr-push-create
description: Commit staged changes, push to fork, and create or update a GitHub PR against the upstream repo using gh CLI. ONLY use when the user explicitly asks to create a PR, push a PR, or update a PR. NEVER activate as part of other tasks.
---

# PR Push & Create

Commit staged changes, push to the user's fork (`origin`), and create or update
a pull request against the upstream (main) repo using `gh` CLI.
...
1. git diff --staged --stat
   -> If empty: print "Nothing staged. Stage your changes first." and STOP.
...
- **NEVER stage files.** The user handles staging. Only commit what is already staged.
